...
#  Loop control: If we increment past last column, reset column counter and increment row counter
#                If we increment past last row, we're finished.
         subi     $s1, $s1, 1    # increment column counter
         bne      $s1, $0, loop1 # not at end of row so loop back
         # println
	 ori     $v0, $0, 4
	 la $a0, newLine			
	 syscall
         move     $s1, $zero     # reset column counter
         subi     $s0, $s0, 1    # increment row counter
         bne      $s0, $0, loop1 # not at end of matrix so loop back
